...
void print_dbt_on_metadata_gstring(struct db_table *dbt, GString *data){
  ...
  if (!quote_character_written) {
    if (identifier_quote_character == BACKTICK)
      ...
      g_string_append_printf(data,"quote_character = DOUBLE_QUOTE\n");
    quote_character_written= TRUE;
  }
...
// TODO: this should be deleted on future releases. 
  ...
  /* FIXME: for different product server_version value is different */
  nroutines= server_version >= 100300 ? array_elements(routine_type) : 2;
