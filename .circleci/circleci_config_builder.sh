...
echo "
  prepare_el_mysql80:
    steps:
    - run: sudo yum install -y libasan gdb screen mysql-community-libs mysql-community-devel mysql-community-client
  ...
  prepare_ubuntu_percona57:
    ...
    - run: sudo apt-get install -y gdb screen libperconaserverclient20 percona-server-client-5.7 libperconaserverclient20-dev

  prepare_ubuntu_percona80:
    steps:
    - run: sudo percona-release setup -y ps80
    - run: sudo apt-get install -y gdb screen libperconaserverclient21 libperconaserverclient21-dev percona-server-client


  prepare_ubuntu_mariadb1006:
    steps:
    - run: sudo apt-get install -y gdb screen mariadb-client libmariadbclient18 libmariadb-dev libmariadb-dev-compat || true
    - run: sudo apt-get install -y gdb screen mariadb-client libmariadbclient18 libmariadb-dev libmariadb-dev-compat

  prepare_el_mariadb1006:
    steps:
    - prepare_mariadb1006
    - run: sudo yum install -y libasan gdb screen MariaDB-devel
    - run: sudo yum install -y libasan gdb screen MariaDB-compat || true
